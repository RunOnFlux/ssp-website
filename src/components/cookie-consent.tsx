...
declare global {
  interface Window {
    ...
    getConsentData?: () => ConsentDebugData
...
interface ConsentDebugData {
  consent?: StoredConsent | null
  analyticsFlag?: string | null
  error?: string
}

export default function CookieConsent() {
  const [showBanner, setShowBanner] = useState(false)
  const [, setPreferences] = useState<StoredConsent>({
  ...
  useEffect(() => {
    ...
    if (!consent) {
      ...
      try {
        const savedPreferences = JSON.parse(consent) as StoredConsent
        if (process.env.NODE_ENV === 'development') {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.hasAnalyticsConsent = () => {
        try {
          ...
          const consentData = JSON.parse(consent) as StoredConsent
      ...
      window.getConsentData = () => {
        try {
          ...
          const analyticsConsent = localStorage.getItem('analytics-consent')
          return {
            consent: consent ? (JSON.parse(consent) as StoredConsent) : null,
            analyticsFlag: analyticsConsent,
          }
        } catch (error) {
